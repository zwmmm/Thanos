...
function deleteScriptFile() {
  try {
    if (files.exists(SCRIPT_PATH)) {
      ...
      console.log('脚本文件已删除:', SCRIPT_PATH);
    }
  } catch (error) {
...
setInterval(function () {
  try {
    // 无障碍权限状态检查
    if (auto.service) {
      ui.wzaService.attr('checked', true);
      isOpenWza = true;
    } else {
      ui.wzaService.attr('checked', false);
      isOpenWza = false;
    }

    if (floaty.checkPermission()) {
      ui.xfcService.attr('checked', true);
      isOpenXfc = true;
    } else {
      ui.xfcService.attr('checked', false);
      isOpenXfc = false;
    }
  } catch (e) {}
}, 1000);
...
function updateScript() {
  ...
  try {
    let response = http.get(
      `https://code.juejin.cn/api/raw/7545469231513485322?id=7545469231513534474`,
      {},
      (res, err) => {
        if (err) {
          toast('更新脚本失败');
...
function register(uts) {
  ...
  http.get(
    `https://activation-code.sanyi.deno.net/projects/5ea0225f-105b-4917-80e0-81f1b8812eab/E0AC192C-7F78-4850-AB26-242F675EB7E0?uks=${uts}`,
    { headers: { uid: userId } },
